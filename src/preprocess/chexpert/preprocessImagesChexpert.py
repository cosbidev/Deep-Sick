...
def preprocess_brax(args):
    ...
    """
    chunks_of_images, dimension, output_dir, skip_if_exists, split = args
    ...
    for img_path in tqdm(chunks_of_images):
        ...
        sub_dir_filename = img_path.split(split+'/')[-1]
...
def parse_arguments():
    ...
    parser.add_argument('--data_path', type=str, default='/mimer/NOBACKUP/groups/naiss2023-6-336/Deep-Sick/data/chexpert-public',
                        help='Path to the raw data of padchest')
    parser.add_argument('--output_path', type=str, default='/mimer/NOBACKUP/groups/naiss2023-6-336/Deep-Sick/data/chexpert-public',
...
def main():
    ...
    print('Started preprocessing of ChexPert')
    ...
    for split in ['train', 'valid', 'test']:
        ...
        dirs_images = [entry for entry in os.scandir(split_path) if entry.is_dir()]
        ...
        for dir in tqdm(dirs_images, desc='Processing directories', unit='dir'):
            ...
            images_list = [str(p) for p in root_dir.rglob("*") if p.suffix.lower() == ".jpg" or p.suffix.lower() == ".jpeg"]
            #image_list_to_process.extend(images_list)
            for image_path in images_list:
                # Check if the image already exists in the output directory
                sub_dir_filename = image_path.split(f'{split}/')[-1]
                dest_img_path = os.path.join(output_dir_split, sub_dir_filename)
        ...
        worker_args = [(chunk, args.dimension, output_dir_split, skip_if_exists, split) for chunk in image_list_chunks]
        # Multiprocessing
        with Pool(processes=num_processes) as pool:
            pool.map(preprocess_brax, worker_args)

        print('Finished preprocessing of ChexPert')
