...
#Muliple run
bash src/bash/multiple_linear.sh -d CAR -v 5
bash src/bash/multiple_linear.sh -d CAR -v 5 -e VANILLA
bash src/bash/multiple_linear.sh -d CAR -v 5 -e FSL
...
# DEBUG MODE:
python src/eval/classification/linear.py experiment/databases@db=CAR experiment/paths/system@_global_=alvis experiment/validation_strategy@_global_=loCo experiment=linear_probing experiment/models@_global_=resnet_50
...
# Post PROCESS
python src/postprocess/aggregate_results/aggregate_results.py experiment/databases@db=CAR experiment/paths/system@_global_=alvis
