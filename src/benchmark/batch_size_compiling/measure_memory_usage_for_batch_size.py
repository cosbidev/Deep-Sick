...
def process_latex_ocr_data(examples, tokenizer, max_length=2048):
    """
    ...
    texts = examples["text"]  # LaTeX formulas

    # Create instruction format for better training
    formatted_texts = []
    for text in texts:
        formatted_text = f"Convert this LaTeX formula to text: {text}"
...
def main_gemma_3b(batch_size):
    ...
    tokenized_dataset = dataset.map(
            lambda examples: process_latex_ocr_data(examples, tokenizer),
            batched=True,
